...
class TestWithCourseCatalogApiMixin(object):
    ...
    """
    def _make_catalog_api(self):
        """
        Build CourseCatalogApiClient mock and register it for deactivation at cleanup.
        :return:
        """
        patched_api = mock.Mock(CourseCatalogApiClient)
        patcher = mock.patch('enterprise.admin.forms.CourseCatalogApiClient', mock.Mock(return_value=patched_api))
        patcher.start()
...
@mark.django_db
@ddt.ddt
class TestManageLearnersForm(unittest.TestCase, TestWithCourseCatalogApiMixin):
    """
    Tests for ManageLearnersForm.
    """

    def setUp(self):
        super(TestManageLearnersForm, self).setUp()
        self.catalog_api = self._make_catalog_api()
    ...
    def test_validate_program_and_course_mode_http_error(self, exception):
        ...
        assert not form.is_valid()
        assert form.errors == {
            "__all__": [ValidationMessages.FAILED_TO_OBTAIN_MODES.format(program_title=program.get("title"))]
        }
...
@mark.django_db
class TestEnterpriseCustomerAdminForm(unittest.TestCase, TestWithCourseCatalogApiMixin):
    """
    ...
    def setUp(self):
        ...
        self.patched_api = self._make_catalog_api()
    ...
    def test_interface_displays_selected_option(self):
        self.patched_api.get_all_catalogs.return_value = [
            {
                "id": self.catalog_id,
                "name": "My Catalog"
            },
